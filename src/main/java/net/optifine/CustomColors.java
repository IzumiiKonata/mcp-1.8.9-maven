...
public class CustomColors {
    ...
    public static void updateReddustFX(EntityFX fx, IBlockAccess blockAccess, double x, double y, double z) {
        if (redstoneColors != null) {
            ...
            int l = j >> 8 & 255;
            int i1 = j & 255;
            float f = (float) k * 0.003921568627451F;
            float f1 = (float) l * 0.003921568627451F;
            float f2 = (float) i1 * 0.003921568627451F;
            fx.setRBGColorF(f, f1, f2);
        }
    }
    ...
    public static void updateWaterFX(EntityFX fx, IBlockAccess blockAccess, double x, double y, double z, RenderEnv renderEnv) {
        if (waterColors != null || blockColormaps != null || particleWaterColor >= 0) {
            ...
            if (particleWaterColor >= 0) {
                ...
                f *= (float) i1 * 0.003921568627451F;
                f1 *= (float) j1 * 0.003921568627451F;
                f2 *= (float) k1 * 0.003921568627451F;
            }
        ...
        for (Object o : props.keySet()) {
            ...
            if (s.startsWith(prefix)) {
                ...
                if (enumdyecolor1 != null && j >= 0) {
                    float[] afloat = new float[]{(float) (j >> 16 & 255) * 0.003921568627451F, (float) (j >> 8 & 255) * 0.003921568627451F, (float) (j & 255) * 0.003921568627451F};
